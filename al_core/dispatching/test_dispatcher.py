...
def test_dispatch_file(clean_redis):
    ...
    with mock.patch('al_core.dispatching.dispatcher.Scheduler', Scheduler):
        ...
        dh.fail_dispatch(file_hash, 'extract')
        ...
        [service_queue(name).delete() for name in disp.scheduler.services]
        ds.error.next_searches.append({'items': [Error({}, docid='error_key')]})
        ds.error.next_searches.append({'items': []})
        ds.error.next_searches.append({'total': 10})
