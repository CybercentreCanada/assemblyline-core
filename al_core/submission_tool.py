...
class SubmissionTool:
    ...
    def __init__(self, datastore: AssemblylineDatastore=None, transport=None, redis=None):
        self.log = logging.getLogger('assemblyline.submission_tool')
    ...
    def submit(self, file_locations: Dict[str, str], metadata: dict, params: SubmissionParams, file_names: Dict[str, str]=None):
        ...
        try:
            ...
            for file_hash, local_file in file_locations.items():
                # Upload/download, extract, analyze files
                size, new_metadata = self._ready_file(file_hash, local_file, expiry, classification)
                metadata.update(new_metadata)

                # Check that after we have resolved exactly what to pass on, that it
                # remains a valid target for scanning
                if size > max_size and not params.ignore_size:
                ...
                accepted_files.append({
                    'name': safe_str(file_names.get(file_hash, file_hash)),
                    'sha256': file_hash,
                })
            ...
            for path in file_locations.values():
    ...
    def _ready_file(self, sha256: str, local_path: str, expiry, classification) -> Tuple[int, dict]:
        ...
        """
        massaged_path = temporary_path = None
        try:
            assert_valid_sha256(sha256)

            # If we don't have the file locally, download it, and vice versa
            if not local_path:
            ...
            elif not self.transport.exists(sha256):
                self.transport.put(local_path, sha256)
            # Now the file should exist locally and in filestore.
            ...
            # If we downloaded anything delete it
            if temporary_path:
